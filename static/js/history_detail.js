document.addEventListener('alpine:init', () => {
    Alpine.data('historyDetail', () => ({
        ...
        init() {
            ...
            this.formattedDate = date.toLocaleString('en-US', { dateStyle: 'full', timeStyle: 'long' });
            ...
            if (detailContentContainer) {
                detailContentContainer.innerHTML = `
                    <div class="card error-card bg-bg-secondary border border-danger-red rounded-xl p-8 shadow-lg text-center py-8">
                        <h2 class="text-2xl font-semibold border-b border-danger-red pb-4 mb-6 flex items-center justify-center gap-3 text-danger-red">
                            <i class="fa-solid fa-triangle-exclamation"></i> History Data Not Found
                        </h2>
                        <p class="text-text-primary text-lg mb-8">It seems the data for this history item is missing or was not selected.</p>
                ...
                document.body.innerHTML = `
                    <div class="container mx-auto p-6 max-w-7xl w-full text-center py-16">
                        ...
                        <p class="text-xl text-text-secondary mt-4 mb-8">History data not found. Please return to the main page.</p>
                        <a href="/" class="btn btn-primary inline-flex items-center px-6 py-3 text-base font-semibold rounded-lg bg-accent-blue text-white hover:bg-accent-blue-hover transition-all duration-200">Back to Dashboard</a>
                    </div>
                `;
            }
        },
        ...
        renderDetailContent() {
            ...
            document.getElementById('detail-heading').innerHTML = `<i class="fa-solid fa-file-invoice text-accent-blue"></i>History Detail #${String(this.item.id).slice(-6)}`;
            document.getElementById('detail-timestamp').textContent = `Created on: ${this.formattedDate}`;
            let contentHTML = `
                <div class="result-detail-item">
                    <h2 class="section-heading text-2xl font-semibold border-b border-border-color pb-4 mb-6 flex items-center gap-3 text-text-primary"><i class="fa-solid fa-keyboard text-accent-blue"></i>Input Data</h2>
                ...
                ${this.item.analysis_type !== 'euclidean' ? `
                    <div class="result-detail-item">
                        <h2 class="section-heading text-2xl font-semibold border-b border-border-color pb-4 mb-6 flex items-center gap-3 text-text-primary"><i class="fa-solid fa-table-cells text-accent-blue"></i>Calculation Result</h2>
                        <div class="table-wrapper rounded-lg overflow-hidden border border-border-color">
                            <div class="table-container overflow-x-auto p-0">
                ...
                ${this.item.dist_matrix_html ? `
                    <div class="result-detail-item">
                        <h2 class="section-heading text-2xl font-semibold border-b border-border-color pb-4 mb-6 flex items-center gap-3 text-text-primary"><i class="fa-solid fa-calculator text-accent-blue"></i>Distance Matrix</h2>
                        <div class="table-wrapper rounded-lg overflow-hidden border border-border-color">
                            <div class="table-container overflow-x-auto p-0">
                                ${this.item.dist_matrix_html}
                            </div>
                ...
                ${this.item.plot_url ? `
                    <div class="result-detail-item">
                        <div class="flex justify-between items-center border-b border-border-color pb-4 mb-6">
                            <h2 class="section-heading text-2xl font-semibold flex items-center gap-3 text-text-primary"><i class="fa-solid fa-chart-line text-accent-blue"></i>Clustering Dendrogram</h2>
                            <button class="btn-icon w-10 h-10 rounded-lg bg-bg-tertiary border border-border-color text-text-secondary hover:text-text-primary hover:border-border-light hover:bg-[#30363D] transition-all duration-200" title="Enlarge Dendrogram" x-data @click="window.dispatchEvent(new CustomEvent('open-image-modal', { detail: { imageUrl: 'data:image/png;base64,${this.item.plot_url}' } }))">
                                <i class="fa-solid fa-expand"></i>
                            </button>
                        </div>
                        <div class="dendrogram-wrapper rounded-xl p-6 bg-bg-secondary border border-border-color text-center overflow-x-auto">
                            <img src="data:image/png;base64,${this.item.plot_url}" alt="Clustering Dendrogram" class="block max-w-full h-auto mx-auto rounded-lg shadow-md">
                        </div>
                    </div>` : ''}
            `;
            const detailContentContainer = document.getElementById('detail-content');
